...
public class CustomAppsActivity extends AppCompatActivity {
    ...
    private void fackData() {
        ...
        List<AppUtils.AppInfo> infoList = AppUtils.getAppsInfo();
        for (AppUtils.AppInfo info : infoList) {
            AppItem item=new AppItem(info.getPackageName(),AppItem.TYPE_DEFAULT);
            appItems.add(item);
        }
